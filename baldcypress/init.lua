...
if minetest.get_modpath("hades_fences") ~= nil then
	local fence = {
		...
		material = "hades_baldcypress:wood",
		groups = {choppy = 2, oddly_breakable_by_hand = 2, flammable = 2},
		sounds = hades_sounds.node_sound_wood_defaults(),
	}
	hades_fences.register_fence("hades_baldcypress:fence", table.copy(fence)) 
	fence.description = S("Bald Cypress Fence Rail")
	--hades_fences.register_fence_rail("hades_baldcypress:fence_rail", table.copy(fence))
	
	if minetest.get_modpath("doors") ~= nil then
		fence.description = S("Bald Cypress Fence Gate")
		doors.register_fencegate("hades_baldcypress:gate", table.copy(fence))
	end
end
