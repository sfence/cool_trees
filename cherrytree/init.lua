...
if minetest.get_modpath("hades_fences") ~= nil then
	local fence = {
		...
		material = "hades_cherrytree:wood",
		groups = {choppy = 2, oddly_breakable_by_hand = 2, flammable = 2},
		sounds = hades_sounds.node_sound_wood_defaults(),
	}
	hades_fences.register_fence("hades_cherrytree:fence", table.copy(fence)) 
	fence.description = S("Cherry Tree Fence Rail")
	--hades_fences.register_fence_rail("hades_cherrytree:fence_rail", table.copy(fence))
	
	if minetest.get_modpath("doors") ~= nil then
		fence.description = S("Cherry Tree Fence Gate")
		doors.register_fencegate("hades_cherrytree:gate", table.copy(fence))
	end
end
