...
minetest.register_craftitem(":chestnuttree:fruit", {
	...
	on_use = minetest.item_eat(2),
	groups = {flammable = 2, food = 2, eatable = 2},
})
...
if minetest.get_modpath("hades_fences") ~= nil then
	local fence = {
		...
		material = "hades_chestnuttree:wood",
		groups = {choppy = 2, oddly_breakable_by_hand = 2, flammable = 2},
		sounds = hades_sounds.node_sound_wood_defaults(),
	}
	hades_fences.register_fence("hades_chestnuttree:fence", table.copy(fence)) 
	fence.description = S("Chestnut Tree Fence Rail")
	--hades_fences.register_fence_rail("hades_chestnuttree:fence_rail", table.copy(fence))
	
	if minetest.get_modpath("doors") ~= nil then
		fence.description = S("Chestnut Tree Fence Gate")
		doors.register_fencegate("hades_chestnuttree:gate", table.copy(fence))
	end
end
